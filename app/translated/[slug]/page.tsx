...
import { ReadingSettings, TranslatedEssay } from '@/lib/types'
...
export default function TranslatedEssayPage({ params }: PageProps) {
  const resolvedParams = use(params)
  const essay = getEssayBySlug(resolvedParams.slug)
  
  if (!essay || essay.type !== 'translated') {
    notFound()
  }

  const translatedEssay = essay as TranslatedEssay
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem(`progress-${resolvedParams.slug}`)
    if (savedProgress) {
  ...
  useEffect(() => {
    localStorage.setItem(`progress-${resolvedParams.slug}`, currentParagraph.toString())
  }, [currentParagraph, resolvedParams.slug])
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      } else if (e.key === 'ArrowRight' && currentParagraph < translatedEssay.paragraphs.length - 1) {
        setCurrentParagraph(prev => prev + 1)
      }
    }
  ...
  }, [currentParagraph, translatedEssay.paragraphs.length])
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <header className="mb-12">
          <h1 className="text-3xl md:text-4xl font-bold mb-2 text-white">
            {translatedEssay.koreanTitle}
          </h1>
          <h2 className="text-xl md:text-2xl text-gray-400 italic mb-4">
            {translatedEssay.title}
          </h2>
          <div className="flex items-center gap-4 text-gray-500">
            <span>{translatedEssay.date}</span>
            <span>•</span>
            <span>{translatedEssay.author}</span>
          </div>
        </header>
        ...
        <div className="mb-8 p-4 bg-gray-900 rounded-lg border border-gray-800">
          <div className="flex items-center justify-between mb-4">
            <span className="text-gray-400">
              진행률: {currentParagraph + 1} / {translatedEssay.paragraphs.length}
            </span>
          ...
          <div className="w-full bg-gray-800 rounded-full h-2">
            <div
              ...
              style={{ width: `${((currentParagraph + 1) / translatedEssay.paragraphs.length) * 100}%` }}
            />
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          {settings.showEnglish && (
            <div className={`p-6 bg-gray-900 rounded-lg border border-gray-800 ${!settings.showKorean ? 'md:col-span-2' : ''}`}>
              ...
              <p className={`text-gray-200 leading-relaxed ${getFontSizeClass()}`}>
                {translatedEssay.paragraphs[currentParagraph].english}
              </p>
            </div>
          )}
          ...
          {settings.showKorean && (
            <div className={`p-6 bg-gray-900 rounded-lg border border-gray-800 ${!settings.showEnglish ? 'md:col-span-2' : ''}`}>
              ...
              <p className={`text-gray-200 leading-relaxed ${getFontSizeClass()}`}>
                {translatedEssay.paragraphs[currentParagraph].korean}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-gray-400">
            {currentParagraph + 1} / {translatedEssay.paragraphs.length}
          </span>

          <button
            onClick={() => setCurrentParagraph(prev => Math.min(translatedEssay.paragraphs.length - 1, prev + 1))}
            disabled={currentParagraph === translatedEssay.paragraphs.length - 1}
            className="px-6 py-3 bg-gray-800 text-gray-200 rounded-lg hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
          >
            다음
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
